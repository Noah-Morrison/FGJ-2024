...
func _input(event):
	if Input.is_action_pressed("move_up") && can_move:
		if position.y < 100 or Global.move_counter >= Global.total_moves:
			pass
		else:
			position.y -= 105
			rotation = 0
	...
	if Input.is_action_pressed("move_down") && can_move:
		if position.y > 400 or Global.move_counter >= Global.total_moves:
			pass
		else:
			position.y += 105
			rotation = 3.2
	...
	if Input.is_action_pressed("move_left") && can_move:
		if position.x < 100 or Global.move_counter >= Global.total_moves:
			pass
		else:
			position.x -= 105
			rotation = 4.7
	...
	if Input.is_action_pressed("move_right") && can_move:
		if position.x > 400 or Global.move_counter >= Global.total_moves:
			pass
		else:
			position.x += 105
			rotation = 1.5
